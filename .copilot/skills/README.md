# VS Code Copilot Skills for AI-Assisted Coding

This directory contains VS Code Copilot Skills - specialized knowledge packs that provide on-demand expertise for specific development domains. Unlike instruction files that are always loaded into context, skills are invoked when needed, making them more context-efficient.

> **Note**: As of VS Code documentation updates, `.copilot/skills/` is the recommended directory for skills. The `.claude/skills/` directory is still supported for backward compatibility, but new projects should use `.copilot/skills/`.
...
| Aspect           | Instructions (`.github/instructions/`)   | Skills (`.copilot/skills/`)    |
| ---------------- | ---------------------------------------- | ------------------------------ |
| **Location**     | `.github/instructions/*.instructions.md` | `.copilot/skills/*/SKILL.md`   |
...
1. **For GitHub Copilot** (Recommended):
   ...
   - Skills are automatically detected from `.copilot/skills/` directory (preferred)
   - Also supports `.claude/skills/` for backward compatibility

2. **For Claude/Anthropic**:
   - Skills are loaded from `.copilot/skills/`, `.claude/skills/`, and `.github/skills/` directories
...
1. **Create skill directory**: `mkdir -p .copilot/skills/my-skill`
